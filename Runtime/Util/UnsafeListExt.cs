...
namespace Barmetler.RoadSystem.Util
{
    public static class UnsafeListExt
    {
        ...
        public static void AddGrowth<T>(this ref UnsafeList<T> ul, in T element, float growth = 1.5f)
            where T : unmanaged
        {
        ...
        public static void AddGrowth<T>(this ref NativeList<T> ul, in T element, float growth = 1.5f)
            where T : unmanaged
        {
            if (ul.Length == ul.Capacity)
            {
